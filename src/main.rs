...
fn extension_is(path: &Path, s: &str) -> bool {
    path.extension().map(|a| a.eq_ignore_ascii_case(s)).unwrap_or(false)
}
...
fn infer_args_mode(arg: &Args) -> Option<Mode> {
    ...
    if let Some(path) = arg.output.as_ref() {
        ...
        if json || toml { Some(Mode::Decode) } else if dat { Some(Mode::Encode) } else { None }
    } else {
        let path = arg.input.as_path();
        let json = extension_is(path, "json");
        let toml = extension_is(path, "toml");
        let dat = extension_is(path, "dat");
        if json || toml { Some(Mode::Encode) } else if dat { Some(Mode::Decode) } else { None }
    }
}
...
fn infer_args_format(arg: &Args, mode: &Mode) -> Option<Format> {
    ...
    match mode {
        Mode::Encode => {
            ...
            let json = extension_is(path, "json");
            let toml = extension_is(path, "toml");
            if json { Some(Format::Json) } else if toml { Some(Format::Toml) } else { None }
        }
        Mode::Decode => {
            arg.output.as_ref().map(|a| a.as_path())
                .and_then(|path| {
                    let json = extension_is(path, "json");
                    let toml = extension_is(path, "toml");
                    if json { Some(Format::Json) } else if toml { Some(Format::Toml) } else { None }
...
fn main() -> anyhow::Result<()> {
    ...
    let format = match arg.format {
        Some(format) => format,
        None => infer_args_format(&arg, &mode).ok_or(anyhow::anyhow!("Unable to infer format from arguments"))?,
    };
    let mut input_reader = if matches!(arg.input.to_str(), Some("-")) {
        BufReader::new(Either::Left(std::io::stdin().lock()))
    } else {
        BufReader::new(Either::Right(File::open(arg.input).context("Opening input file")?))
    };
...
fn decode(format: Format, reader: &mut impl Read, writer: &mut impl Write) -> anyhow::Result<()> {
    ...
    writer.write_all(serialized.as_bytes()).context("Writing output")
}
...
fn encode(format: Format, reader: &mut impl Read, writer: &mut impl Write) -> anyhow::Result<()> {
    ...
    codec::Settings::from_simple(&deserialized).encode_to_writer(writer).context("Encoding settings")
}
